<?php /*a:3:{s:78:"C:\phpStudy\PHPTutorial\WWW\03kj\shop\application\admin\view\product\list.html";i:1563528894;s:72:"C:\phpStudy\PHPTutorial\WWW\03kj\shop\application\admin\view\header.html";i:1563505848;s:72:"C:\phpStudy\PHPTutorial\WWW\03kj\shop\application\admin\view\footer.html";i:1562661169;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
<meta charset="utf-8">
...
<section class="Hui-article-box">
	<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 产品管理 <span class="c-gray en">&gt;</span> 货品属性 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
		...
		<article class="cl pd-20">
			<div class="text-c">
					<input  hidden="" id="__token__" name="__token__" value="<?php echo htmlentities(app('request')->token()); ?>">
					 ...
					 <button type="button" class="btn btn-success" id="add" name="" onClick=""><i class="Hui-iconfont">&#xe600;</i> <a href="<?php echo url('product/add'); ?>">添加</a></button>
...
show()
	...
	function modaldemo(id,goods_id,goods_attr_id,price,sn_number){
		$("#up_id").val(id)	
		$("#up_goods_id").val(goods_id)
		$("#up_goods_attr_id").val(goods_attr_id)
		$("#up_price").val(price)
		$("#up_sn_number").val(sn_number)
			...
			data:{
				...
				goods_attr_id:goods_attr_id,
					...
					for (var i = 0; i <data.length; i++){
						tr=tr+"<tr><td><input class='mycheckbox' type='checkbox' value='"+data[i]['id']+"'</td><td>"+data[i]['id']+"</td><td>"+data[i]['g_name']+"</td><td>"+data[i]['g_s_id']+"<a href='<?php echo url("goodsattr/list"); ?>?id="+data[i]['id']+"'><i class='Hui-iconfont'>&#xe604;</i></a></td>><td>"+data[i]['price']+"</td>><td>"+data[i]['sn_number']+"</td><td class='f-14 product-brand-manage'><a style='text-decoration:none' onclick='modaldemo(\""+data[i]['id']+"\",\""+data[i]['g_name']+"\",\""+data[i]['g_s_id']+"\",\""+data[i]['price']+"\",\""+data[i]['sn_number']+"\")' title='编辑'><i class='Hui-iconfont'>&#xe6df;</i></a> <a style='text-decoration:none' class='delete_one ml-5' onclick='del("+data[i]['id']+")' title='删除'><i class='Hui-iconfont'>&#xe6e2;</i></a></td></tr>";			
					}
					$('.table tbody').html(tr)
			}
		})
	...
	function del(id){
     ...
     function  datadel() {
     	...
     	$.ajax({
     		url:"<?php echo url('attribute/deletemore'); ?>",
     	    type:'post',
     		data:{
     			id:id,
